...
public class App{
    ...
    public static void main(String[] args) {
        ...
        check("BOGO Strategy (1 item)", pricingService.calculatePrice(singleApple) == 10.0);
        check("BOGO Strategy (2 items)", pricingService.calculatePrice(twoApples) == 10.0);
        check("Bulk Strategy (below threshold)", pricingService.calculatePrice(fiveSodas) == 25.0);
        check("Bulk Strategy (above threshold)", pricingService.calculatePrice(tenSodas) == 45.0);
        check("Default Strategy", pricingService.calculatePrice(normalBread) == 60.0);
        ...
        check("Promotion update works", pricingService.calculatePrice(threeApples) == 15.0); // 30 * 0.5 = 15.0
